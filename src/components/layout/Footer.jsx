...
const Footer = () => {
  ...
  return (
    <footer className="mt-6 pt-14 sm:pt-16 pb-8 text-slate-200 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
      <div className="container mx-auto px-4 sm:px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-9">
        <div className="lg:col-span-1 flex flex-col gap-4 pr-0 lg:pr-6">
          ...
          <p className="text-sm text-slate-400 leading-relaxed max-w-[240px]">
            TDM Infra supplies boom lifts, cranes, scissor lifts, and truck
            mounted lifts for industrial and commercial projects that need
            reliable access support.
          </p>
        </div>

        <div className="lg:col-span-1 flex flex-col gap-4 pl-0 lg:pl-6 pt-1 md:pt-0 lg:border-l border-white/10">
          <h3 className="text-base font-medium text-white">TDM Infra </h3>
          <nav className="flex flex-col gap-3" aria-label="Footer navigation">
            {footerColumns.map((link) => (
              <Link
                key={link.label}
                to={link.to}
                className="flex items-center gap-2 text-slate-300 text-sm hover:text-sky-400 transition-colors"
              >
          ...
          <div className="flex flex-col gap-4">
            {contactItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="flex items-center gap-3 text-slate-300 text-sm hover:text-white"
              >
                <span
                  className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-white/10 text-sky-400"
                  aria-hidden="true"
                >
                  {item.icon}
                </span>
      ...
      <div className="container mx-auto px-4 sm:px-6 flex flex-col sm:flex-row justify-between gap-2 sm:gap-4 pt-6 mt-2 border-t border-white/10">
        <p className="text-sm text-slate-500">
          Copyright (c) TDM Infra. All rights reserved.
        </p>
        <p className="text-sm text-slate-500">
          Industrial access equipment, delivered with service.
        </p>
      </div>
    </footer>
  );
